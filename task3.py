...
# this function is for task 3
# Function returns the matched inflection feature description
# it appends the lemma to inflected form for each of the inflection rules,
# whichever matched with correct form, it returns the feature
def get_best_features(lemma, correct, pre_rules_dict, suf_rules_dict, feat=0):
    base = "<" + lemma + ">"
    key_list = []
    for k1, v1 in suf_rules_dict.items():
        key_list.append(k1)
        for k2 in v1:

            test_base = base
            test_base = str.replace(test_base, k2[0], k2[1])
            test_base = str.replace(test_base, '<', '')
            test_base = str.replace(test_base, '>', '')

            # this section is for test file with three column
            if feat != 0:
                if test_base == correct and k1 == feat:
                    # returns the feature description that matches both the inflected form and description rules
                    return k1
            # this section is for test file with two column
            else:
                if test_base == correct:
                    return k1

    # if doesn't find any match, then returns randomly selected feature description
    secure_random = random.SystemRandom()
    return secure_random.choice(key_list)
...
def main(argv):
    ...
    if len(val) == 3:
        for l in testfile:
            ...
            # calling the function to predict the best feature description
            task3_res = get_best_features(lemma, correct, pre_rules_dict, suf_rules_dict, feat)
            ...
            if LINE:
                print(task3_res) # prints the predicted feature description if -l parameter is passed
    ...
    # for test file with two column
        ...
        for l in testfile:
            lemma, correct, = l.split(u'\t')
            if pref_bias > suf_bias:
                lemma = lemma[::-1]

            # calling the function to predict the best feature description
            task3_res = get_best_features(lemma, correct, pre_rules_dict, suf_rules_dict)
